Ext.define('August.view.production.LineController', {
    ...
    onClearFilters: function(b){
        var me = this,
            ...
            searchfield = topbar.down('searchtextlist'),
    ...
    onListOpenSheet: function(item, e){
        ...
        console.log('onList', rec);
        this.redirectTo('line/tab/' + rec.get('lineseq'));
    },
    ...
    onAddToLineSheet: function(c, e){
        ...
        Ext.each(selected, function(rec){
            ctn = ctn + 1;
            linesheet.stylesInLines().add(
                Ext.create('August.model.style.Sil', {
                    seq: ctn,                     
                    style: rec.get('style'),
                    color: rec.get('color'),
                    lineseq: linesheet.lineseq
                })
            );
        });
    ...
    newLineSheet: function(field, picker){
        var store = this.getViewModel().getStore('linesheets'),
            v = picker.getValue(),
            rec = Ext.create('August.model.style.LineSheet', {
                linetitle: field.getValue(),
