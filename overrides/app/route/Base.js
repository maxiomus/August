...
Ext.define('Ext.app.route.Base', {
    ...
    /*createHash: function(tab, id){
        ...
        ha[0] = tab ? '!' + tab : (ha[0] ? ha[0] : August.app.getDefaultToken());
        ...
        if(tab){
            ...
            switch(type){
                case 'multiview':
                    ...
                    //console.log(active, active.getXType(), active.superclass.getXType())
                    
                    if(xtypes.indexOf(active.superclass.getXType()) == -1) {
                        if(xtypes.indexOf(active.getXType()) == -1){
                            active = active.down();
                        }
                    }
                    //console.log(grid, grid.getXType())
                    if(active && active.getSelectionModel().getSelection().length > 0){
                        var p = active.getSelectionModel().getSelection();
                ...
                case 'sales-edit-form':
                    ...
                    break;
                */
               case 'purchase-orderForm':
                    route[1] = tab.opMode;
    
                    if(tvm.get('thePO') != null && tvm.get('thePO').id > 0){
                        route[2] = tvm.get('thePO').id;
                    }
                
                break;
               case 'sales-orderForm':
                    route[1] = tab.opMode;
    
                    if(tvm.get('theOrder') != null && tvm.get('theOrder').id > 0){
                        route[2] = tvm.get('theOrder').id;
                    }
                ...
                case 'payment-receiveForm':
                    ...
                    if(tvm.get('thePayment') != null && tvm.get('thePayment').id > 0){
                        route[2] = tvm.get('thePayment').id;
                    }
                ...
                case 'style-edit-form':
                    ...
                    if(tvm.get('theProduct').id > 0) {
                        route[2] = tvm.get('theProduct').id;
                    }
                    else {
    ...
    addBookmark: function(record, scope, extraData){
        ...
        var bookmark = Ext.create('August.model.Bookmark', {
